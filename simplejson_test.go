...
func TestSetPathNoPath(t *testing.T) {
	js, err := NewSimple([]byte(`{"some":"data","some_number":1.0,"some_bool":false}`))
	assert.Equal(t, nil, err)
...
func TestPathWillAugmentExisting(t *testing.T) {
	js, err := NewSimple([]byte(`{"this":{"a":"aa","b":"bb","c":"cc"}}`))
	assert.Equal(t, nil, err)
...
func TestPathWillOverwriteExisting(t *testing.T) {
	...
	js, err := NewSimple([]byte(`{"this":{"a":0.1,"b":"bb","c":"cc"}}`))
	assert.Equal(t, nil, err)
